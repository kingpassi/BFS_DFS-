...
nput: image = [[1,1,1],[1,1,0],[1,0,1]], sr = 1, sc = 1, color = 2
...
public:
    ...
    void flood_fill(vector<vector<int>>& image, int r, int c, int color,int oldcolor){
        if(r<0 || c<0 || r>=image.size() || c>=image[0].size() || oldcolor!=image[r][c]) 
            return;
        
        image[r][c]=color;
        flood_fill(image,r+1,c,color,oldcolor);
        flood_fill(image,r-1,c,color,oldcolor);
        flood_fill(image,r,c+1,color,oldcolor);
        flood_fill(image,r,c-1,color,oldcolor);
    }
